<script lang="ts">
	...
	const calculateStartPosition = () => {
		...
		return {
			x: $nodes[nodeId].position.x + localCenter.x,
			y: $nodes[nodeId].position.y + localCenter.y
		};
	};
</script>
...
<div
	...
	bind:this={elm}
	on:mousedown={(e) =>
		pathDraw.enableDraw(e, connectorId, connType, direction, calculateStartPosition())}
	on:mouseup={() =>
		pathDraw.disableDraw(connectorId, connType, direction, calculateStartPosition())}
>
	<slot />
</div>
